...
(function () {
    ...
    function get_api(request, response)  {
        var actions = {};
        for (act in REMOTING_ACTIONS) {
            actions[act] = [];
            for (m in REMOTING_ACTIONS[act]) {
        ...
        var api = "Ext.Direct.addProvider(%s);".replace('%s', JSON.stringify(config));
        ...
        var contentType = request.headers['content-type'],
            onEndCallback = function(data) {
                ...
                for (var i in data) {
                    ...
                    var mcfg = REMOTING_ACTIONS[action][method];
                    ...
                    try {
                        ...
                        func.call(callback, request, response, extdirect_post_data);
                    } catch(e) {
